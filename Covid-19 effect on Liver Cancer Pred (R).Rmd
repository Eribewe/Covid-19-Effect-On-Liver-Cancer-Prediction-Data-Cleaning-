---
title: "Covid-19 effect on Liver Cancer Prediction (using R)"
...
All necessary libraries for whatever manipulation that may be reqired is loaded
```{r, include=FALSE}
...
library(caret) #classification/regression model training
...
Loading the data set gotten from https://www.kaggle.com/datasets/fedesoriano/covid19-effect-on-liver-cancer-prediction-dataset
with strings loaded as factors and header set to true as the data comes with headers.
...
barchart(df.Na, xlab = "Count of missing values in the field", ylab = "Fields", main = "Visual of all fields with the number of missing values")
```
Quite a number of the fields have missing values consisting of more than 50% of the field and there is no way we can predict those missing values as there is not enough remaining data in the fields to make such prediction/assumption.
Hence, there is need to remove these fields.
...
summary(df)
```
Now, we can fix up the remaining fields.
#### Need to fix up missing data 
We start by fixing the fields with very small amount of missing values as it won't require too much technicalities
Starting from the size, treatment_grps, PS, and Prev_Known_Cirrhosis fields.
```{r}
